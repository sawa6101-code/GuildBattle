...
const current=await all(d,'partyCharacters');const hasAll=spec.chars.every(x=>current.some(z=>z.party_id===p.id&&z.position===x.pos));
if(!hasAll){for(const old of current.filter(x=>x.party_id===p.id))await del(d,'partyCharacters',old.id);for(const x of spec.chars){const c=cs.find(v=>v.name===x.name);const row={id:'pc_'+crypto.randomUUID(),party_id:p.id,position:x.pos,character_id:c?.id||null,character_name_snapshot:x.name,level:x.level,power:x.power,hp:null,attack:null,defense:null,speed:null,source:'screenshot',source_date:'2026-09-18',recognition_status:c?'visual_match_candidate':'character_not_found',verification_required:!c,updated_at:now()};await put(d,'partyCharacters',row)}}
}
