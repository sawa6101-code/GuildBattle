...
async function match(name,opts={}){
 const chars=opts.characters||await allStore(opts.db||await new Promise((res,rej)=>{const r=indexedDB.open(DB,2);r.onsuccess=()=>res(r.result);r.onerror=()=>rej(r.error)}),'characters');
...
async function confirm(partyCharacterId,characterId){
 const db=await new Promise((res,rej)=>{const r=indexedDB.open(DB,2);r.onsuccess=()=>res(r.result);r.onerror=()=>rej(r.error)});
...
function install(){
 const run=async()=>{
  ...
  const cards=[...box.querySelectorAll('.party')];const parties=await (async()=>{const db=await new Promise((res,rej)=>{const r=indexedDB.open(DB,2);r.onsuccess=()=>res(r.result);r.onerror=()=>rej(r.error)});return allStore(db,'parties')})();
