...
(function(){
 ...
 const battle=['phase4.js','phase4_1.js','phase4_2_character_master.js','phase4_3.js','phase4_4.js','phase4_5.js','phase4_6.js','phase4_7.js','phase4_8.js','phase4_9.js','phase4_10.js'];let battleReady=null;
 ...
 async function ensureBattle(){if(battleReady)return battleReady;status('⚙️ 戦闘機能を読み込み中…');battleReady=series(battle).then(()=>{document.querySelector('#lazyBattleStatus')?.remove()}).catch(e=>{battleReady=null;status('読み込みエラー: '+e.message);throw e});return battleReady}
 ...
 async function prepare(v){if(v==='battle')return ensureBattle();if(v==='characters')return ensureCharacters();if(v==='import'){await loadScript('phase3.js');return ensureOCR()}if(v==='settings')return loadScript('phase5.js')}
 document.addEventListener('click',async e=>{if(replaying)return;const b=e.target.closest('[data-view]');if(!b)return;const v=b.dataset.view;if(!['battle','characters','import','settings'].includes(v))return;e.preventDefault();e.stopImmediatePropagation();try{await prepare(v);replaying=true;b.click();replaying=false}catch(err){console.error(err);replaying=false;status('読み込みに失敗しました。ページを再読み込みしてください。')}} ,true);
 window.ParanoiseLoader={ensureBattle,ensureCharacters,ensureOCR,loaded,prepare};
})();
